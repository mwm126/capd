...
void SHA1HMAC(u8 *chal, int chalLen, u8 *key, int keyLen, u8 *resp) {
  u32 respLen;
  HMAC(EVP_sha1(), (unsigned char *)key, keyLen, (unsigned char *)chal, chalLen,
...
void decryptAES128ECB(u8 *cryptText, int cryptLen, u8 *key, u8 *plainText) {
  ...
  AES_ecb_encrypt((unsigned char *)cryptText, (unsigned char *)plainText,
                  &aeskey, AES_DECRYPT);
  return;
}

void decryptAES256CBC(u8 *cryptText, int cryptLen, u8 *key, u8 *IV,
                      u8 *plainText) {
  AES_KEY aeskey;
  ...
  AES_cbc_encrypt((unsigned char *)cryptText, (unsigned char *)plainText,
                  cryptLen, &aeskey, (unsigned char *)tmpIV, AES_DECRYPT);
  return;
}
