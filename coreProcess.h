...
void coreProcess(int coreToNetSocket, int coreToAuthSocket) {
  ...
  while (1) {
    ...
    if ((read(coreToNetSocket, remoteAddr, sizeof(remoteAddr)) !=
         sizeof(remoteAddr)) ||
    ...
    if (memcmp(plain->serverAddr, serverAddr, addrSize) != 0) {
      char str[33];
      logOutput(lf, 1, "Bad server address in decrypted block.  Serial: %d",
                serial);
    ...
    if (memcmp(plain->authAddr, remoteAddr, addrSize) != 0) {
      char str[33];
      logOutput(lf, 1, "Bad client address in decrypted block.  Serial: %d",
                serial);
    ...
    /* Verify challenge construction */
    {
      ...
      if (memcmp(packet->challenge, digest, SHA256_DIGEST_LENGTH) != 0) {
        logOutput(lf, 1,
                  "Challenge construction verification failed.  Serial: %d",
                  serial);
        continue;
      }
    }
    ...
    /* Verify IV construction */
    {
      ...
      if (memcmp(packet->IV, digest, 16) != 0) {
        logOutput(lf, 1, "IV construction verification failed.  Serial: %d",
                  serial);
        continue;
      }
    }
    ...
    /* Verify OTP */
    {
      ...
      if ((memcmp(digest, OTP->OTPChallenge, 6) != 0)) {
        logOutput(lf, 1, "OTP Challenge verification failed.  Serial: %d",
                  serial);
        continue;
      }
      ...
      if (counter <= searchCounterFile(cf, serial)) {
        logOutput(lf, 1, "OTP Counter verification failed.  Serial: %d",
                  serial);
        continue;
      }
      updateCounterFileEntry(cf, serial, counter);
      logOutput(lf, 1, "Accepted connection: %s, %d, %d", username, serial,
                counter);
    }
