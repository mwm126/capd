...
void authProcess(int authToCoreSocket, int netPID, int corePID) {
  ...
  unsigned int i, port, timeLimit;

  /* This process retains privilege and is unjailed. */
  while (1) {
    /* Wait for authorization information */
    if ((read(authToCoreSocket, allowedAddr, sizeof(allowedAddr)) !=
         sizeof(allowedAddr)) ||
        (read(authToCoreSocket, hostAddr, sizeof(hostAddr)) !=
         sizeof(hostAddr)) ||
        (read(authToCoreSocket, &port, sizeof(port)) != sizeof(port)) ||
        (read(authToCoreSocket, &timeLimit, sizeof(timeLimit)) !=
         sizeof(timeLimit)))
    ...
    for (i = 0; i < sizeof(allowedAddr); i++)
      if (!((allowedAddr[i] == 0) ||  /* Null OK */
            ...
            ((allowedAddr[i] >= 97) &&
             (allowedAddr[i] <= 102)) || /* a-f OK (IPV6) */
            ((allowedAddr[i] >= 65) &&
             (allowedAddr[i] <= 70)) /* A-F OK (IPV6) */
            ))
    ...
    for (i = 0; i < sizeof(hostAddr); i++)
      if (!((hostAddr[i] == 0) ||  /* Null OK */
            ...
            (hostAddr[i] == 45) ||
            (hostAddr[i] == 95) || /* hyphen and underscore */
